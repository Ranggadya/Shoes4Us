...
export class UserService {
    ...
    async verifyToken(token: string) {
        try {
            ...
            throw new AppError('Token tidak valid atau sudah kadaluarsa', 401, 'INVALID_TOKEN');
        }
    }
    ...
    async changePassword(id: string, currentPassword: string, newPassword: string) {
        const user = await prisma.user.findUnique({ where: { id } });
        if (!user) throw new AppError('User tidak ditemukan', 404, 'USER_NOT_FOUND');
        if (!user.password) throw new AppError('Password akun belum tersedia', 400, 'PASSWORD_MISSING');

        const validPassword = await BcryptUtil.comparePassword(currentPassword, user.password);
        if (!validPassword) {
            throw new AppError('Password saat ini tidak sesuai', 401, 'INVALID_CURRENT_PASSWORD');
        }
